...
(function() {
    ...
    const CFG = {
        ...
        INVITES_PER_HOUR: 3,       // max invites per hour
        ...
        RAMP_UP_DAYS: 7,
        DAILY_START_CAP: 10,       // starting daily cap during ramp-up
        MAX_PER_DAY: 20,           // steady-state daily cap
    ...
    function buildUI(){
        ...
        start.onclick = ()=>{ state.running=true; start.disabled=true; stop.disabled=false; status.innerHTML=`<small>running — lastHour:${countLastHour()} today:${countToday()}</small>`; runAutomation(); };
